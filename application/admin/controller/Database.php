    ...
    public function getRecords()
    {
        ...
        switch ($type) {
            ...
            case 'import':
                ...
                $records["recordsFiltered"] = $records["recordsTotal"] = count($list);
                $records["data"] = array_slice($list,$start,$length);
                foreach ($records["data"] as &$row) {
                    $time = $row['time'];
                    $row['time'] = date('Ymd-His',$row['time']);
            ...
            case 'export':
                ...
                $records["data"] = array_slice($list,$start,$length);
                foreach ($records["data"] as &$row) {
                    $row['selectDOM'] = '<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input name="id[]" type="checkbox" class="checkboxes" value="' . $row['name'] . '"/><span></span></label>';
                    $row['data_length'] = format_bytes($row['data_length']);
                    $row['status'] = '<em class="info">未备份</em>';
                }
                break;
            default:
